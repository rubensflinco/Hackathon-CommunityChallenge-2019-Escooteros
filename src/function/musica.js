...
export default class FunctionMusica extends React.Component {
  ...
  efeitoSonoroRapido(url, name = 'efeitoSonoro', muted = false, volume = 0.3, loop = false, autoPlay = false) {
    if (global.ConfigSomMuted == false) {
      this.destroy(name);
      this.create(url, name, muted, volume, loop, autoPlay);
      this.play(name);
    } else {
      this.destroy(name);
    }
  ...
  create(url, name = 'musica', muted = false, volume = 0.3, loop = false, autoPlay = false) {
